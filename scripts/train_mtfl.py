...
from sampling_argmax.trainer import train, validate_gt_mtfl
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if (i + 1) % args.snapshot == 0:
            ...
            with torch.no_grad():
                ...
                abs_err = err['abs_err']
                rel_err = err['rel_err']
                if args.log and abs_err <= best_abs_err:
                    best_abs_err = abs_err
                    torch.save(m.module.state_dict(), './exp/{}-{}/best_abs_model.pth'.format(args.exp_id, cfg.FILE_NAME))
                if args.log and rel_err <= best_rel_err:
                    best_rel_err = rel_err
                    torch.save(m.module.state_dict(), './exp/{}-{}/best_rel_model.pth'.format(args.exp_id, cfg.FILE_NAME))
                logger.info(f'##### Epoch {args.epoch} | Abs Err: {abs_err:2f}/{best_abs_err:2f} | Rel Err: {rel_err:2f}/{best_rel_err:2f} #####')
